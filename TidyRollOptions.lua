...
local c_TROLL_AUTO_ROLL_DISABLE = tabs[c_TROLL_MISC_TAB].windowName .. "AutoRollDisable"
...
function TidyRollOptions.Initialize()
	...
	TidyRollOptions.InitializeMiscTab()
end

function TidyRollOptions.InitializeMiscTab()
	
	
	------------------------------------------------------------------------------------------------------
    -- MISC SETTINGS
    
	local tabWindowName = tabs[c_TROLL_MISC_TAB].windowName
    ...
    -- Disable autoroll in dungeons Setting
	if( CreateWindowFromTemplate(c_TROLL_AUTO_ROLL_DISABLE, "TRollCheckboxTemplate", tabWindowName) ) then
		
		
		WindowAddAnchor(c_TROLL_AUTO_ROLL_DISABLE, "bottomleft", c_TROLL_AUTO_NEED_FOR_MEDALLIONS, "topleft", 0, offset - 10)
		WindowAddAnchor(c_TROLL_AUTO_ROLL_DISABLE, "topright", tabWindowName, "topright", -10, 0)
		
		LabelSetText(c_TROLL_AUTO_ROLL_DISABLE .. "Label", L"Disable autoroll in dungeons")
    end
	...
	if( CreateWindowFromTemplate(c_TROLL_TIMER_TEXT, "TRollCheckboxTemplate", tabWindowName) ) then
		...
		WindowAddAnchor(c_TROLL_TIMER_TEXT, "bottomleft", c_TROLL_AUTO_ROLL_DISABLE, "topleft", 0, 0)
		WindowAddAnchor(c_TROLL_TIMER_TEXT, "bottomright", c_TROLL_AUTO_ROLL_DISABLE, "topright", 0, 0)
		
		LabelSetText(c_TROLL_TIMER_TEXT .. "Label", L"Show timer text")
    end
    
    -- Career Icon Showing Setting
...
function TidyRollOptions.Reset()
	...
	ButtonSetPressedFlag( c_TROLL_AUTO_ROLL_DISABLE .. "Button", GetSetting("auto-roll-disable-in-dungeons") )
...
function TidyRollOptions.OnApply()
	...
	SetSetting( "auto-roll-disable-in-dungeons", ButtonGetPressedFlag(c_TROLL_AUTO_ROLL_DISABLE .. "Button") )
